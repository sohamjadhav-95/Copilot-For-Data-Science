...
function renderPlan(data) {
    ...
    if (nodes.length === 0) {
        stepsEl.innerHTML = '<div style="padding:16px;color:rgba(255,255,255,0.35);font-size:0.8rem;text-align:center;">No steps returned by the planner. Try a more detailed goal.</div>';
    ...
    if (dagLinear) {
        ...
        <div class="dag-step-row" id="dag-step-${i}">
          ...
          <div class="dag-step-body">
            ...
            <div class="dag-step-desc" style="color:rgba(255,255,255,0.35);font-size:0.7rem;">${esc(node.type || '')}</div>
          </div>
        </div>`).join('');
    }
    ...
    <div class="output-block">
      ...
      <div class="output-block-body" style="white-space:pre-wrap;font-size:0.8rem;color:rgba(255,255,255,0.7);">${userGoal ? esc(userGoal) : 'Plan generated — review steps and approve to execute.'}</div>
      ${nodes.length > 0 ? `
      <div style="padding:8px 14px 12px;border-top:1px solid rgba(255,255,255,0.06);">
        ${nodes.map((n, i) => `
        <div style="display:flex;align-items:flex-start;gap:8px;margin-bottom:8px;">
          ...
          <div>
            <div style="font-size:0.8rem;color:rgba(255,255,255,0.8);font-weight:500;">${esc(n.description || n.operation || '')}</div>
            <div style="font-size:0.7rem;color:rgba(255,255,255,0.3);">${esc(n.type || '')}</div>
          </div>
        </div>`).join('')}
      </div>` : ''}
    </div>`);
...
function buildTableHtml(data) {
    if (typeof data === 'string') {
        try { data = JSON.parse(data); } catch (e) { return `<pre style="color:rgba(255,255,255,0.7);font-size:0.75rem;overflow-x:auto;">${esc(data)}</pre>`; }
    }
    ...
    if (!cols.length) return '<em style="color:rgba(255,255,255,0.3);">Empty result</em>';

    const tableId = 'wf-tbl-' + Date.now();
    return `
    <div style="margin-bottom:8px;display:flex;align-items:center;gap:8px;">
      <div style="position:relative;flex:1;max-width:220px;">
        <span class="material-symbols-rounded" style="position:absolute;left:8px;top:50%;transform:translateY(-50%);font-size:14px;color:rgba(255,255,255,0.3);pointer-events:none;">search</span>
        <input type="text" placeholder="Filter rows..." oninput="wfFilterTable('${tableId}',this.value)"
          style="width:100%;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:6px;padding:5px 10px 5px 28px;color:rgba(255,255,255,0.75);font-size:0.75rem;outline:none;font-family:var(--font-sans);">
      </div>
      <span style="font-size:0.7rem;color:rgba(255,255,255,0.3);">${rows.length} rows</span>
    </div>
    <div style="overflow-x:auto;max-height:320px;overflow-y:auto;">
      <table class="data-table" id="${tableId}" style="background:transparent;">
        <thead>
          <tr>
            ${cols.map((c, ci) => `<th class="sortable" onclick="wfSortTable('${tableId}',${ci})" style="background:rgba(255,255,255,0.06);color:rgba(255,255,255,0.5);border-color:rgba(255,255,255,0.08);">${esc(String(c))}<span class="sort-indicator"></span></th>`).join('')}
          </tr>
        </thead>
        <tbody>
          ${rows.slice(0, 200).map(row => `<tr>${(Array.isArray(row) ? row : cols.map(c => row[c])).map(v => `<td style="border-color:rgba(255,255,255,0.05);color:rgba(255,255,255,0.65);">${esc(String(v ?? ''))}</td>`).join('')}</tr>`).join('')}
        </tbody>
      </table>
      ${rows.length > 200 ? `<div style="padding:6px 10px;font-size:0.7rem;color:rgba(255,255,255,0.3);">Showing 200 of ${rows.length} rows</div>` : ''}
    </div>`;
}
