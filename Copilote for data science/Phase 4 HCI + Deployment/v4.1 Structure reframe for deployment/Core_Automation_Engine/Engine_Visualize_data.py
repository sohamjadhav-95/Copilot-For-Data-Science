...
def Groq_Input(user_input):
    try:
        ...
        system_prompt = f"""
        You are an expert Python Data Scientist. Your task is to generate Python code to **VISUALIZE** data.
        You can use libraries like `matplotlib`, `seaborn`, or `plotly`.
        ...
        **Rules:**
        ...
        2. **Analysis**: Infer the best visualization type for the user's request.
        3. **Plotting**: Generate the plot and use `plt.show()` (for matplotlib/seaborn) or `fig.show()` (for plotly).
...
def execute_generated_code(generated_code, user_input):
    ...
    try:
        print("\nExecuting Visualization Operation...\n")
        exec(valid_code)
        print("\nTask completed successfully!")
        result_response(user_input, "Visualization operation completed.")
    except Exception as e:
        print(f"\nExecution Error: {e}")
...
def generate_code_error_handling(user_input, failed_code, error):
    try:
        ...
        system_prompt = f"""
        You are an expert Python Debugger. The previous visualization code failed. Fix it.
        ...
        **Rules:**
        ...
        completion = client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": system_prompt},
                {"role": "user", "content": "Fix the visualization code."}
            ],
            temperature=0.2,
            max_tokens=4096,
            top_p=0.95,
        ...
        if code_match:
            ...
            exec(fixed_code)
            print("\nTask completed successfully!")
            result_response(user_input, "Fixed and executed visualization operation.")
        else:
